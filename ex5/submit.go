...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	for _, lang_id := range formData.Langs {
		req = fmt.Sprintf("INSERT INTO languages_on_user (user_id, lang_id) VALUES (%d, %s);", last_user_id, lang_id)
		output, err = send_sql_request(req)
	}
	...
	req = fmt.Sprintf("INSERT INTO sensitives (user_id, login, pass) VALUES (%d, '%s', '%s');", last_user_id, login, hex.EncodeToString(hashed_pass[:]))
